...
namespace GaussianSplatting.Editor
{
    ...
    public class GaussianSplatAssetCreator : EditorWindow
    {
        ...
        [SerializeField] bool m_ImportFromFolder = true;
        ...
        private string folderPath = "Select a folder";

        void BrowseFolder()
        {
            GUILayout.Label("Select a Folder", EditorStyles.boldLabel);
            m_ImportFromFolder = EditorGUILayout.Toggle("Import From Folders", m_ImportFromFolder);
            ...
            EditorGUILayout.LabelField("Selected Path:", folderPath);
        }
        ...
        void OnGUI()
        {
            ...
            m_InputFile = m_FilePicker.PathFieldGUI(rect, new GUIContent("Input PLY/SPZ File"), m_InputFile, "ply,spz",
                "PointCloudFile");
            ...
            {
                if (!m_ImportFromFolder)
                    CreateAsset(m_InputFile, false);
                else
                    CreateAssetAtOnce();
            }
        ...
        void CreateAssetAtOnce()
        {
            ...
            filesToProcess = new List<FileInfo>();
            filesToProcess.AddRange(dir.GetFiles("*.ply"));
            filesToProcess.AddRange(dir.GetFiles("*.spz"));

            currentIndex = 0;

            AssetDatabase.DisallowAutoRefresh();

            // 🔥 Start ONE file per frame
            EditorApplication.delayCall += ProcessNextFile;
        }


        void ProcessNextFile()
        {
            if (filesToProcess == null || currentIndex >= filesToProcess.Count)
            {
                FinishAll();
                Debug.Log("All files imported successfully.");
                return;
            }

            EditorUtility.DisplayProgressBar(
                "Batch Import",
                $"Importing {currentIndex + 1}/{filesToProcess.Count}",
                (float)currentIndex / filesToProcess.Count
            );

            try
            {
                CreateAsset(filesToProcess[currentIndex].FullName, false);
            }
            catch (Exception e)
            {
                Debug.LogError(e);
            ...
            // 🔥 Schedule NEXT file in NEXT frame
            EditorApplication.delayCall += ProcessNextFile;
        }
        ...
        /*void CreateAssetAtOnce()
        {
            if (m_ImportFromFolder)
            {
                ...
                {
                    FileInfo f = filesToProcess[i];
                    m_InputFile = $"{folderPath}/{f.Name}";

                    // Update Progress Bar
                    EditorUtility.DisplayProgressBar("Batch Processing",
                        $"Processing {i+1}/{total}: {f.Name}",
                        (float)i / total);
                    ...
                    */ /*if (cancel)
                    ...
                    try
                    {
                        // Pass 'false' to prevent selecting the object in the project window
                        // (which loads it into memory)
                        CreateAsset(m_InputFile, false);
                    }
                    catch (Exception ex)
                    {
                        Debug.LogError($"Failed to process {f.Name}: {ex.Message}");
                    }

                    // CRITICAL MEMORY FIX:
                    // Force Unity to release the huge TextAssets/Meshes we just loaded
                    if (i % 5 == 0 || i == total - 1) // Clean up every 5 files or at the end
                    {
                ...
                EditorUtility.ClearProgressBar();
                AssetDatabase.SaveAssets(); // One final save
                Debug.Log($"Batch processing of {total} files completed.");
            }
            else
            {
                CreateAsset(m_InputFile, true);
            }
        }*/
        unsafe void CreateAsset(string plyFileName, bool selectAssetAfterCreation = true)
        {
            m_ErrorMessage = null;
            // Use the passed argument, fallback to member variable if needed
            string currentInputFile = !string.IsNullOrEmpty(plyFileName) ? plyFileName : m_InputFile;

            if (string.IsNullOrWhiteSpace(currentInputFile))
            {
            ...
            // Removed DisplayProgressBar here to avoid conflict with the Batch Loop progress bar
            // using simple logs if in batch mode, or checks
            if (selectAssetAfterCreation) EditorUtility.DisplayProgressBar(kProgressTitle, "Reading data files", 0.0f);

            GaussianSplatAsset.CameraInfo[] cameras = LoadJsonCamerasFile(currentInputFile, m_ImportCameras);
            using NativeArray<InputSplatData> inputSplats = LoadInputSplatFile(currentInputFile);

            if (inputSplats.Length == 0)
            {
                if (selectAssetAfterCreation) EditorUtility.ClearProgressBar();
                return;
            }
            ...
            if (selectAssetAfterCreation) EditorUtility.DisplayProgressBar(kProgressTitle, "Morton reordering", 0.05f);
            ...
            if (m_FormatSH >= GaussianSplatAsset.SHFormat.Cluster64k)
            {
                if (selectAssetAfterCreation) EditorUtility.DisplayProgressBar(kProgressTitle, "Cluster SHs", 0.2f);
                ClusterSHs(inputSplats, m_FormatSH, out clusteredSHs, out splatSHIndices);
            }

            string baseName = Path.GetFileNameWithoutExtension(FilePickerControl.PathToDisplayString(currentInputFile));
            ...
            // NOTE: This refresh is heavy, but necessary to load the TextAssets below.
            AssetDatabase.Refresh(ImportAssetOptions.ForceUncompressedImport);

            if (selectAssetAfterCreation)
                EditorUtility.DisplayProgressBar(kProgressTitle, "Setup data onto asset", 0.95f);

            // This loads the data into Memory. This is where the RAM usage spikes.
            asset.SetAssetFiles(
            ...
            if (selectAssetAfterCreation) EditorUtility.DisplayProgressBar(kProgressTitle, "Saving assets", 0.99f);
            AssetDatabase.SaveAssets();
            ...
            // Important: Drop reference to the asset so GC can pick it up in the loop
            asset = null;
            savedAsset = null;
        }
        ...
        static unsafe void ClusterSHs(NativeArray<InputSplatData> splatData, GaussianSplatAsset.SHFormat format,
            out NativeArray<GaussianSplatAsset.SHTableItemFloat16> shs, out NativeArray<int> shIndices)
        {
            ...
            GatherSHs(splatData.Length, (InputSplatData*)splatData.GetUnsafeReadOnlyPtr(),
                (float*)shData.GetUnsafePtr());

            NativeArray<float> shMeans = new(shCount * kShDim, Allocator.Persistent);
            shIndices = new(splatData.Length, Allocator.Persistent);

            KMeansClustering.Calculate(kShDim, shData, kBatchSize, passesOverData, ClusterSHProgress, shMeans,
                shIndices);
            shData.Dispose();
            ...
            Debug.Log(
                $"GS: clustered {splatData.Length / 1000000.0:F2}M SHs into {shCount / 1024}K ({passesOverData:F1}pass/{kBatchSize}batch) in {t1 - t0:F0}s");
        }
        ...
        struct CreateOtherDataJob : IJobParallelFor
        {
            ...
            public NativeArray<int> m_SplatSHIndices;
        ...
        void CreateColorData(NativeArray<InputSplatData> inputSplats, string filePath, ref Hash128 dataHash)
        {
            ...
            NativeArray<float4> data = new(width * height, Allocator.Persistent);
        ...
        static void EmitSimpleDataFile<T>(NativeArray<T> data, string filePath, ref Hash128 dataHash)
            where T : unmanaged
        {
            dataHash.Append(data);
            using var fs = new FileStream(filePath, FileMode.Create, FileAccess.Write);
