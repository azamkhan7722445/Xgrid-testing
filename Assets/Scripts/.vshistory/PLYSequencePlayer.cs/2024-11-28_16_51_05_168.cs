...
public string filePrefix = "frame"; // File prefix
    ...
    {// Set the frame time
        ...
        nextMesh = LoadPLY(GetFileName((currentFrame + 1) %
        frameCount));
        StartCoroutine(PlaySequence());
    }
    I
    Enumerator PlaySequence()
    {
        while (isPlaying)
        {
            ...
            if (currentMesh != null && nextMesh != null)
            {
                ...
                while (interpolationProgress < 1f)
                {
                    ...
                    frameTime;
                    meshFilter.mesh = InterpolateMeshes(currentMesh,
                    nextMesh, interpolationProgress);
                    yield return null;
                }
            } /
    ...
    p
    rivate Mesh InterpolateMeshes(Mesh meshA, Mesh meshB, float t)
    {
        ...
        {
            interpolatedVertices[i] = Vector3.Lerp(verticesA[i],
            verticesB[i], t);
        } /
        ...
        {
            interpolatedNormals[i] = Vector3.Lerp(normalsA[i],
            normalsB[i], t).normalized;
        } /
        / Set interpolated data to the mesh
    interpolatedMesh.vertices = interpolatedVertices;
        interpolatedMesh.normals = interpolatedNormals; interpolatedMesh.triangles = meshA.triangles; // Assume same
        topology
    return interpolatedMesh;
    }
    p
    rivate Mesh LoadPLY(string filePath)
    {
    ...
    p
    rivate string GetFileName(int frameIndex)
    {
        return
        $"{folderPath}/{filePrefix}{frameIndex:D3}{fileExtension}";
    }
}